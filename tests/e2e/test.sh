...
FOOBAR_ITEM='{"url":"https://next.obudget.org/s/?q=foobar","title":"foobar","properties":{"a":1,"b":[1,2]}}'
JWT_TOKEN="mock_jwt"
TEST_USER_ID=123
...
# add items
[ '{"item_id": 1, "list_id": 1}' != "$(list_curl PUT "${TEST_USER_ID}" "list=saved-searches" "-d ${FOOBAR_ITEM}")" ] \
    && echo "failed to add foobar item to saved search list" && exit 1

[ '{"item_id": 2, "list_id": 1}' != "$(list_curl PUT "${TEST_USER_ID}" "list=saved-searches" "-d ${HEALTH_SERVICES_ITEM}")" ] \
    && echo "failed to add health services item to saved search list" && exit 1

[ '{"item_id": 3, "list_id": 1}' != "$(list_curl PUT "${TEST_USER_ID}" "list=saved-searches" "-d ${ACQUISITIONS_AND_MAINTANANCE_ITEM}")" ] \
    && echo "failed to add acqusitions and maintanence item to saved search list" && exit 1

[ '{"item_id": 4, "list_id": 1}' != "$(list_curl PUT "${TEST_USER_ID}" "list=saved-searches" "-d ${TEST_ITEM}")" ] \
...
[ '{"a":1,"b":[1,2]}' != "$(echo "${ITEMS_RES}" | jq -c '.items[0].properties')" ] \
    && echo "didn't get expected properties of item 0" && exit 1
